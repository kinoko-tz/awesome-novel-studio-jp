...
2. デフォルト値（`[create]` セクションがない場合）:
   ```
   draft_chars: 3000前後            # Phase 2 初稿目標（多めに作成）
   final_chars: 2500以内             # Step 2.5 トリミング後の最終目標
   ...
   > **分量戦略: 「あふれるほど書いて削る」**
   > episode-creator は `draft_chars`（3000 字前後）を目標に初稿を書く。
   > Step 2.5 でオーケストレーターが `final_chars`（2500 字以内、目標 2000-2500 字）にトリミングする。
...
- プロンプトに含める情報:
  ...
  - **目標分量の明示**（必ず含める）:
    ...
    novel-config.md の [create] セクション値がない場合はデフォルト値 2,000～2,500 字を使用する。
    ```
  - パス制限指示（上記共通指示を含める）
...
**戦略: 「あふれるほど書いて削る」**
episode-creator は 3000 字前後の初稿を作成する。オーケストレーターが 2500 字以内にトリミングする。
不足して肉付けするより、十分に書いて削る方が品質が高い。

**測定基準**: `wc -m` でファイル全体の文字数を測定する（Markdown ヘッダ・メタデータを含む）。

1. **測定**: Bash `wc -m {EPISODE_DIR}/ep{NNN}.md` → 文字数を確認
2. **判定**: 最終目標範囲（デフォルト final_chars: 2000-2500）と照合する

   - **上限超過**（2500 字超 — 一般的なケース）:
   ...
   - **範囲内**（2000-2500 字）: Phase 3 へ進行

   - **下限未満**（2000 字未満 — 初稿が不足する異常ケース）:
...
3. Bash: `wc -m {EPISODE_DIR}/ep{NNN}.md` → 目標文字数範囲（デフォルト final_chars: 2000-2500）を確認
...
dead_zone_threshold: 1200        # ポイント場面なしの最大許容文字数（デフォルト: 1200）
